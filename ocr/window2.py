...
from fanzhuan import mirror_rotate_image
...
def predict():#这个函数用于预测用户绘制的字母。首先将当前画布保存为一个临时PNG文件，然后加载该文件并转换为模型需要的张量格式。接着使用LeNet-5模型对输入图像进行预测，得到预测结果并显示在一个消息框中。
    ...
    filename = os.path.join(os.path.dirname(filename) + "\\"+ "rotated_" + os.path.basename(filename))
    ...
    with torch.no_grad():
        ...
        messagebox.showinfo("Prediction Result", f'The predicted number is {predicted_class}.')
